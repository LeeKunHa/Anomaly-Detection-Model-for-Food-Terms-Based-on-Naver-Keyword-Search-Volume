...
def main():
    ...
    #keyword_list = _cfg['Keyword']['User_list'] #하나씩 사용도 가능
    #공백제거(한달치 검색량에 안 잡힘)
    ...
    #검색량 상대값 저장
    df.to_csv(f'data/result/search_result_relative.csv', index=False, encoding='cp949')
    
    
    #불러오기2(상대값이 정상적으로 저장되었다면 여기서부터!!)
    df = pd.read_csv(f'data/result/search_result_relative.csv',encoding='cp949')
    ...
    #검색량 절대값 저장
    final_df.to_csv(f'data/result/search_result_absolute.csv', index=False, encoding='cp949')
